...
2. **Deduplication** : exact URL / URL without ignored params / includes modes; keep strategy (`keep-grouped` default, `keep-old`, `keep-new`) to decide which of the two matching tabs survives; undo action captures `groupId` and tries to restore group membership of the closed tab
