...
A click or pressing `Enter` uses the **default action** configured in [Settings](/guides/settings#default-restore).

### Conflict wizard \{#conflict}
...
<Aside type="note">
Tabs restored through the wizard are protected against immediate deduplication for 10 seconds (same mechanism as [Undo on deduplication](/guides/deduplicate#undo)).
</Aside>
...
## Multi-selection and bulk actions \{#bulk}
...
<CardGrid>
  ...
  <LinkCard title="Settings" href="/guides/settings" description="Default restore action, notifications." />
