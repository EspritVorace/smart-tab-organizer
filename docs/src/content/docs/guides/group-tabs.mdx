...
When you open a link from a parent tab, the extension detects the action, consults your [domain rules](/guides/domain-rules), and drops the new tab into a colored Chrome group with no manual action.
...
## Joining an existing group \{#join-existing}
...
When several rules match the same domain, the rule placed highest in your list wins. You can reorder via drag and drop on the [Domain Rules](/guides/domain-rules) page.
...
- **Global level**: the **Grouping** toggle in the popup or the [Settings](/guides/settings) page. When it is off, an orange **G** badge appears on the extension icon.
...
<CardGrid>
  <LinkCard title="Domain Rules" href="/guides/domain-rules" description="Create, edit and organize the rules that drive grouping." />
  <LinkCard title="Deduplicate" href="/guides/deduplicate" description="The complementary mechanism that avoids duplicates." />
  <LinkCard title="Categories and packs" href="/reference/categories-and-packs" description="Import ready-to-use rules with their regexes and colors." />
  <LinkCard title="Regex presets" href="/reference/regex-presets" description="The ready-made regex templates from the creation wizard." />
</CardGrid>
