...
Quand vous ouvrez un lien déjà ouvert dans votre navigateur, SmartTab Organizer conserve un seul onglet et ferme l'autre. La stratégie "quel onglet garder" (voir plus bas) détermine lequel survit. Par défaut, l'onglet qui appartient à un groupe est prioritaire, sinon c'est l'onglet existant qui reste focusé.
...
- **L'onglet groupé gagne, sinon conserver l'existant** (défaut) : si l'un des deux onglets fait partie d'un groupe et l'autre pas, le groupé survit. Sinon retombe sur le comportement "L'onglet existant". Cette option protège notamment les onglets restaurés depuis une session (qui portent leur groupe) contre une fermeture au profit d'une copie déjà ouverte en dehors du groupe, et évite qu'un clic sur un lien ne fasse sauter l'onglet hors de son groupe.
