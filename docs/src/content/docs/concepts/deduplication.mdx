...
Quand vous ouvrez un lien déjà ouvert dans votre navigateur, SmartTab Organizer conserve un seul onglet et ferme l'autre. La stratégie "quel onglet garder" (voir plus bas) détermine lequel survit. Par défaut, c'est l'onglet existant qui reste, et le nouvel onglet est fermé avec focus sur l'ancien.
...
- **L'onglet existant** (défaut) : comportement historique, l'onglet déjà ouvert est conservé et le nouveau fermé. Utile quand le tab existant a un état (scroll, formulaire, session) que vous ne voulez pas perdre.
- **Le nouvel onglet ouvert** : le nouvel onglet survit, l'ancien est fermé. Utile quand la version fraîchement chargée doit primer.
