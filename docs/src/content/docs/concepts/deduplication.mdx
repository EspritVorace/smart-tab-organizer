...
Quand vous ouvrez un lien déjà ouvert dans votre navigateur, SmartTab Organizer conserve un seul onglet et ferme l'autre. La stratégie "quel onglet garder" (voir plus bas) détermine lequel survit. Par défaut, l'onglet qui appartient à un groupe est prioritaire, sinon c'est le nouvel onglet qui reste (la version fraîchement chargée).
...
- **Désactivé** (défaut) : les onglets des domaines sans règle ne sont jamais dédupliqués, même si le toggle global est actif.
...
- **L'onglet existant** : l'onglet déjà ouvert est conservé et le nouveau fermé. Utile quand le tab existant a un état (scroll, formulaire, session) que vous ne voulez pas perdre.
...
- **L'onglet groupé gagne, sinon conserver l'existant** : si l'un des deux onglets fait partie d'un groupe et l'autre pas, le groupé survit. Sinon retombe sur "L'onglet existant".
- **L'onglet groupé gagne, sinon conserver le nouveau** (défaut) : si l'un des deux onglets fait partie d'un groupe et l'autre pas, le groupé survit. Sinon retombe sur "Le nouvel onglet ouvert". Ce mode protège les onglets groupés (restaurés depuis une session ou cliqués via un lien) contre une fermeture au profit d'une copie non groupée, tout en privilégiant le contenu fraîchement chargé quand la question du groupe ne tranche pas.
