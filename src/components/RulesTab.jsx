    ...
    return (
        <section id="rules-section">
            ...
            {groupOrder.map(groupId => {
                ...
                return (
                    <div className="rules-group-container" key={groupId}>
                        <div
                            className={`rules-group-header ${group.color ? `group-color-${group.color}` : ''} ${getTextContrastClass(group.color)} ${isExpanded ? 'expanded' : ''}`}
                            onClick={() => toggleGroupCollapse(groupId)}
                        >
                            <span className="group-arrow">{isExpanded ? '▼' : '▶'}</span>
                            <span className="group-label">{group.label}</span>
                            <span className="rule-count">({rulesInGroup.length} {rulesInGroup.length === 1 ? getMessage('ruleCountSingular', 'rule') : getMessage('ruleCountPlural', 'rules')})</span>
                        </div>
                        <div className={`rules-group-content ${isExpanded ? 'expanded' : ''}`}>
            ...
            {ungroupedRules.length > 0 && 
                <div className="rules-group-container" key="_ungrouped">
                    <div
                        className={`rules-group-header ungrouped-rules-header ${!collapsedGroups['_ungrouped'] ? 'expanded' : ''}`}
                        onClick={() => toggleGroupCollapse('_ungrouped')}
                    >
                        <span className="group-arrow">{!collapsedGroups['_ungrouped'] ? '▼' : '▶'}</span>
                        <span className="group-label">{getMessage('ungroupedRules', 'Ungrouped Rules')}</span>
                         <span className="rule-count">({ungroupedRules.length} {ungroupedRules.length === 1 ? getMessage('ruleCountSingular', 'rule') : getMessage('ruleCountPlural', 'rules')})</span>
                    </div>
                    <div className={`rules-group-content ${!collapsedGroups['_ungrouped'] ? 'expanded' : ''}`}>
            ...
            {!editingId && !newRuleInProgress && <button onClick={handleAdd} className="button add-button">{getMessage('addRule')}</button>}
        </section>
    );
}
    ...
    return (
        <div className="list-item">
            <div className="item-view">
                ...
                <label htmlFor={`enable-${rule.id}`} className="item-details">
                    <span className={`item-main ${disabledClass}`}>{rule.label}</span>
                    <span className={`item-sub ${disabledClass}`}>
                        {subtitleParts.join(' | ')}
                    </span>
                </label>
                <div className="item-actions">
                    <button onClick={() => onEdit(rule.id)}>{getMessage('edit')}</button>
                    <button onClick={() => onDelete(rule.id)} className="danger">{getMessage('delete')}</button>
                </div>
            </div>
        </div>
    );
                        ...
                        {formData.groupNameSource === 'title' && 
                            <div className="form-group tooltip-container full-width">
                                ...
                                <select value={currentRegexValue} onChange={handleSelectChange}>
                                    {presets.map(p => <option key={p.regex} value={p.regex}>{p.name}</option>)}
                        ...
                        {formData.groupNameSource === 'url' && 
                            <div className="form-group tooltip-container full-width">
                                ...
                                <select value={currentUrlRegexValue} onChange={handleUrlSelectChange}>
                                    {presets.map(p => <option key={p.urlRegex} value={p.urlRegex}>{p.name}</option>)}
                                    <option value="custom">{getMessage('customRegex')}</option>
                                </select>
